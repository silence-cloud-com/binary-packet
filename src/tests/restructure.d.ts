// The restructure package doesn't have types...sadly
// This type definition was copied from https://github.com/foliojs/restructure/issues/55
...
declare module 'restructure' {
  ...
  export class Array<T, N extends number | string> extends Base<LengthArray<BaseOf<T>, N>> {
    type: T
    length?: N
  ...
  export class Enum<T extends readonly string[]> extends Base<T[number] | number> {
  ...
  export type StructType<T extends Record<string, unknown>> = {
    [K in keyof T]: T[K] extends Base<infer U> ? U : T[K] extends () => infer V ? V : T[K]
  }

  export type StructFields<T extends Record<string, unknown>> = {
    [K in keyof T]: T[K] extends () => infer V ? (this: StructType<Omit<T, K>>) => V : T[K]
  }
  ...
  export class Struct<T extends Record<string, unknown>> extends Base<StructType<T>> {
